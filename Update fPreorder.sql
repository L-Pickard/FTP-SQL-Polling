...
ALTER PROCEDURE [dbo].[Update fPreorder]
AS
BEGIN
	SET XACT_ABORT ON;
	...
	BEGIN TRY
		...
		FROM OPENROWSET(BULK '\\WHServer\\Users\leo.pickard\Desktop\Misc Bulk Inserts\Preorder Data\upload preorders.xml', SINGLE_BLOB) AS T(x)

		DROP TABLE

		IF EXISTS [dbo].[fPreorder];
			CREATE TABLE [dbo].[fPreorder] (
		...
		CREATE NONCLUSTERED INDEX IX_fPreorder_Preorder_Item ON [dbo].[fPreorder] (
			[Preorder Code]
			,[Item No]
			) INCLUDE (
			[Customer No]
		...
		CREATE NONCLUSTERED INDEX IX_fPreorder_Customer_Date ON [dbo].[fPreorder] (
			[Customer No]
			,[Order Timestamp]
			) INCLUDE (
			[Preorder Code]
		...
		ALTER INDEX ALL ON [dbo].[fPreorder] REBUILD;

		UPDATE STATISTICS [dbo].[fPreorder];

		--The below code enters a success record into the update log table
		SELECT @Timestamp = SYSDATETIME()
			,@Level = 'INFO'
			,@File_Name = 'Update fPreorder.sql'
			,@Table = 'fPreorder'
			,@Action = 'Update fPreorder table with new data.'
			,@Row_Count = (
				SELECT COUNT(*)
				
				FROM [Warehouse].[dbo].[fPreorder]
				)
			,@Start_Date = NULL
			,@End_Date = NULL
			,@Message = 'New data has been written to fPreorder table from owtanet ftp csv.';
